...
final class ChurchAdminController
{
    ...
    public function store(): void
    {
        ...
        if(!Validator::text($name,2,100)||!Validator::email($email)||strlen($password)<10||!in_array($role,['owner','admin','content_manager'],true)){$this->fail('이름, 이메일, 10자 이상 비밀번호와 역할을 확인해 주세요.');}
    ...
    public function suspend(int $userId): void
    {
        ...
        try {$repo->suspendForTenant($tenant,$userId);} catch(RuntimeException $e){$this->fail($e->getMessage());}
    ...
    private function fail(string $message): never {Session::flash('error',$message);Response::redirect('/admin/managers');}
}
