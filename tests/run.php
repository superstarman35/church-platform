...
check(str_contains($invitationController, "preg_match('/^[=+\\-@\\t]/u'") && str_contains($invitationController, 'csvCell('), 'application CSV neutralizes spreadsheet formulas');
...
check(str_contains($mediaRepo, 'UPDATE invitation_media SET deleted_at=NOW()') && str_contains($mediaRepo, 'church_id=:church_id') && str_contains($mediaRepo, 'invitation_id=:invitation_id'), 'gallery deletion is recoverable and tenant scoped');
...
check(str_contains($subscriptionRequests, "['awaiting_payment','rejected']") && str_contains($subscriptionRequests, "status='pending'"), 'platform review only moves a pending request to payment review or rejection');
check(str_contains(source($root . '/resources/views/control/dashboard.php'), '실제 입금·결제 확인 뒤 참조값'), 'platform review explains payment confirmation safety');
